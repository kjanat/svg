...
| **P8** | One command (`just refresh-spec`) + CI auto-PR on schedule                           | **PARTIAL** - `just regen`/`regen-sample`/`regen-test` recipes exist; scheduled freshness workflow + auto-refresh PR path are in place; single `just refresh-spec` wrapper still TODO |
