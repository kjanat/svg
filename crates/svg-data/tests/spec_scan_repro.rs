...
/// Locate the svgwg `master/` directory to scan.
///
/// Defaults to the single vendored checkout under
/// `data/sources/svgwg-*/master`. The `SPEC_SCAN_MASTER_OVERRIDE` env var
/// points the scan at an alternate `master/` dir — used to prove the port
/// reproduces the committed JSON against the *pristine* upstream bytes
/// when the vendored copy has been reformatted.
fn vendored_master() -> PathBuf {
    if let Ok(override_path) = std::env::var("SPEC_SCAN_MASTER_OVERRIDE") {
        return PathBuf::from(override_path);
    }
    let sources = manifest_dir().join("data/sources");
    let Ok(read_dir) = std::fs::read_dir(&sources) else {
        panic!("data/sources is not readable: {}", sources.display());
    };
    let entry = read_dir.filter_map(Result::ok).map(|e| e.path()).find(|p| {
        p.file_name()
            .and_then(|n| n.to_str())
            .is_some_and(|n| n.starts_with("svgwg-"))
    });
    let Some(entry) = entry else {
        panic!("no vendored svgwg-* directory under {}", sources.display());
    };
    entry.join("master")
}
