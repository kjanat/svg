...
struct CompatMetadata {
    schema_version: u16,
    #[serde(default)]
    browser_compat_data: PackageSource,
    #[serde(default)]
    web_features: PackageSource,
    #[serde(default)]
...
fn empty_catalog() -> String {
    [
        ...
        "pub static COMPAT_SOURCES: &[(&str, &str, &str)] = &[];",
...
fn emit_catalog(
    ...
    out.push_str("pub static COMPAT_SOURCES: &[(&str, &str, &str)] = &[\n");
    if let Some(compat) = compat {
        for source in [&compat.browser_compat_data, &compat.web_features] {
            let _ = writeln!(
                out,
                "({:?}, {:?}, {:?}),",
