...
fn live_gaps() -> Fallible<BTreeSet<String>> {
    let catalog: Value = serde_json::from_str(&fs::read_to_string(catalog_core_path())?)?;
    Ok(catalog["attributes"]
        .as_array()
        .ok_or("catalog.core.json has no `attributes` array")?
        .iter()
...
fn baseline_gaps() -> Fallible<BTreeSet<String>> {
...
#[test]
fn coverage_gaps_match_baseline() -> Fallible<()> {
    let live = live_gaps()?;
