...
/** Compact display keeps the upstream version string, including qualifiers. */
export function browserVersionChipLabel(version: BrowserVersion): string {
	return typeof version.version_added === 'string' ? version.version_added : '—';
}
...
function countBrowserSignals(
	...
	for (const entry of entries) {
		...
		for (const key of BROWSER_KEYS) {
			const v = selectBrowserStatement(support[key]);
			if (v === undefined) continue;
			if (v.partial_implementation) hasPartial = true;
			if (v.version_removed !== undefined) hasRemoved = true;
			if (v.flags.length > 0) hasFlagged = true;
			if (v.version_added === false) hasUnsupported = true;
		}
...
function browserTokens(entry: CompatEntry): string[] {
	...
	for (const key of BROWSER_KEYS) {
		...
		if (version === undefined) continue;
		tokens.push(key);
		if (version.version_added === false) anyUnsupported = true;
		else if (typeof version.version_added === 'string') {
			anySupported = true;
		}
		...
		if (version.flags.length > 0) anyFlagged = true;
	}
